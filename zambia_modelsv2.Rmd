...
train_data_polynomials2 <- cbind(train_data, c_breastf2 = I(train_data$c_breastf^2), c_age2 = I(train_data$c_age^2), m_agebirth2 = I(train_data$m_agebirth^2), m_height2 = I(train_data$m_height^2), m_bmi2 = I(train_data$m_bmi^2), c_age_x_breastf = train_data$c_age*train_data$c_breastf, c_age_x_breastf2 = I((train_data$c_age*train_data$c_breastf)^2))

test_data_polynomials2 <- cbind(test_data, c_breastf2 = I(test_data$c_breastf^2), c_age2 = I(test_data$c_age^2), m_agebirth2 = I(test_data$m_agebirth^2), m_height2 = I(test_data$m_height^2), m_bmi2 = I(test_data$m_bmi^2), c_age_x_breastf = test_data$c_age*test_data$c_breastf, c_age_x_breastf2 = I((test_data$c_age*test_data$c_breastf)^2))
...
The model found by step AIC is slightly better than the previous polynomial regression with degree 2, and it suggests to discard m_agebirth and to avoid squaring m_bmi, it also maintains the predictors that were previously dropped.
...
Because of the really high GVIF value we decided to drop some variables: c_breastf, m_height, c_age2, m_height2.

```{r lm_polynomials2_gvif}
lm_polynomials2_gvif <- lm(zscore ~ c_gender + c_age + m_bmi + m_education + 
    region + c_breastf + m_agebirth2, data = train_data_polynomials2)
...
Now let's try by using the categorical version of c_breastf on the best polynomial regression found.

```{r lm_polynomials2_cat}
lm_polynomials2_cat <- lm(zscore ~ c_gender + c_breastf + c_age + m_height + m_bmi + m_education + 
    region + I(c_age^2) + I(m_agebirth^2) + I(m_height^2) + 
    c_age*c_breastf, data = train_data_cat)

summary(lm_polynomials2_cat)
```

```{r test_error_polynomials_2_cat}
predicted <- predict(lm_polynomials2_cat, newdata = test_data_cat)
MSE <- mean((predicted - test_data_cat$zscore)^2)
RMSE <- sqrt(MSE)
MAE <- mean(abs(predicted - test_data_cat$zscore))
...
AIC <- AIC(lm_polynomials2_cat)
BIC <- BIC(lm_polynomials2_cat)

paste("AIC: ", round(AIC), ", BIC: ", round(BIC))
```

This model behave slightly better than the previous one, and it has lower BIC and AIC, so we will keep it as the best one.
...
gam_c_breastf_cat <- gam(zscore ~ c_gender + c_breastf + s(c_age) + s(m_agebirth) + s(m_height) + s(m_bmi) + m_education + region, data = train_data_cat)
...
gam_linear_height_bmi <- gam(zscore ~ c_gender + s(c_breastf) + s(c_age) + s(m_agebirth) + m_height + m_bmi + m_education + region, data = train_data)
...
anova(gam_null, gam_most_significant, gam_linear_height_bmi, test = "F")
```
...
The best Polynomial Regression model found with step AIC, it's the one that has degree 2, uses all predictors except m_agebirth, doesn't consider the square of m_bmi and considers interaction between c_breastf and c_age and uses the categorical version of c_breastf:

* MSE:  15337.69, RMSE:  123.84, MAE:  90.21
* AIC:  46696 , BIC:  46833
